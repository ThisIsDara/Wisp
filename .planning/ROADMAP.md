...
### Phase 8: Auto-updater: GitHub release check, notify-first update flow, settings section
...
### Phase 9: Performance: buttery smooth search and scroll with large inventories
...
### Phase 10: Search system re-implementation - Omarchy/Walker-inspired provider architecture, fully async, instant
...
### Phase 11: Next-version backlog: accent-follow chrome, Alt+number quick-select, cmd/ command runner

**Goal:** Ship the next-version backlog (user-written): (1) the app's chrome — outer border + halo + every separator hairline — derives from the selected accent instead of fixed neon-orange; (2) Alt+number quick-select with visible 1-9 row hints on typed queries; (3) a typed command runner where `cmd/<command>` executes in a new cmd.exe console on Enter.
**Requirements**: D-01..D-13 (.planning/BACKLOG.md + 11-CONTEXT.md)
**Depends on:** Phase 10
**Plans:** 2 plans

Plans:

- [ ] 11-01 accent-follow chrome + Alt+number quick-select (QML: Theme.qml, ResultsRow.qml, MainWindow.qml)
- [ ] 11-02 cmd/ command runner (C++: CommandProvider, Source::Command, WinLaunch::launchCommand, LaunchController branch, wiring, tests)

### Phase 12: Expose app keyboard-shortcuts reference: "Show shortcuts" button in Settings opening a themed shortcuts window

**Goal:** Let users discover wisp's keyboard shortcuts via a "Show shortcuts" button in the Settings window that opens a themed reference window listing all app shortcuts.
**Requirements**: TBD
**Depends on:** Phase 11
**Plans:** 1 plan
...
- [ ] 12-01-PLAN.md — ShortcutsWindow + Settings Show-shortcuts row + Settings polish (header-on-top, Remove buttons, hairlines) + Theme tokens + tests
